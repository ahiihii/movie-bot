...
async def setup_commands(app):
    commands = [BotCommand("start", "Khởi động bot")]
...
async def search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not keyword:
        return
    wait_msg = await update.message.reply_text("🔎 Đang tìm phim...")
    found_any = False
    ...
    for sid in ["1", "2", "3"]:
        try:
            ...
            data = r.json()
            movies = data.get("items") if sid == "1" else data.get("data", {}).get("items", [])
            if movies:
                found_any = True
                keyboard = [
                    [InlineKeyboardButton(m.get("name", "Không tên"), callback_data=f"{sid}|{m.get('slug','')}")]
                    for m in movies[:5]
                ]
                ...
                if sid == "2":
                    server_label += " (Vietsub + Thuyết Minh)" if "thuyết minh" in movie_text or "thuyet minh" in movie_text else " (Vietsub)"
                if sid == "3":
                    server_label += " (Vietsub + Lồng Tiếng)" if "lồng tiếng" in movie_text or "long tieng" in movie_text else " (Vietsub)"

                await update.message.reply_text(server_label, reply_markup=InlineKeyboardMarkup(keyboard))
        except Exception as e:
            print(f"SERVER {sid} ERROR:", e)

    if not found_any:
        await wait_msg.edit_text("❌ Không tìm thấy phim!")
    else:
...
async def movie_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        detail_url = SOURCES[source_id]["detail"] + slug
        r = await client.get(detail_url)
        data = r.json()
        movie = data.get("movie", {})
        episodes = data.get("episodes", []) or movie.get("episodes", [])
